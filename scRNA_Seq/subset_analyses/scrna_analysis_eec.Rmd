...
# 4 = SIN N Nts
# 5 = SILA I Cck
# 6 = SAKD D Sst and Iapp
```

```{r}
scrna_eec$sub_annotation <- scrna_eec$RNA_snn_res.0.4

Idents(scrna_eec) <- "sub_annotation" 

new_names <- c("EC","Early EEC","EC-Reg4","K","N","I","D")
names(new_names) <- levels(scrna_eec)

scrna_eec <- RenameIdents(scrna_eec,new_names)

levels(scrna_eec$sub_annotation) <- new_names

eec_umap <- DimPlot(scrna_eec, group.by = "sub_annotation",
                    reduction = "harmony_UMAP",
                    cols = "Set3") & mini_umap_theme & ggtitle(element_blank())

eec_umap_name <- DimPlot(scrna_eec, 
                         reduction = "harmony_UMAP", 
                         group.by = "stage") & mini_umap_theme & ggtitle(element_blank())
...
markers_new <- c("Tph1","Chga","Chgb","Sct",
                 "Reg4","Gip","Nts","Cck",
                 "Sst")
...
dp_new_marker <- DotPlot_scCustom(
      ...
      legend.key.spacing = unit(0.001,"cm")
  )
...
#Barplot and Proportions
```{r}
generate_prop_table(scrna = scrna_eec,
                    table_vars = c("stage","sub_annotation"),
                    name = "eec_inf_noc2",
                    plots_folder = plots_folder )

generate_prop_table(scrna = scrna_eec_uninf,
                    table_vars = c("stage","sub_annotation"),
                    name = "eec_uninf_noc2",
                    plots_folder = plots_folder )


cluster_proportion <- t(prop.table(x = table(scrna_eec_uninf$sub_annotation,
                                                  scrna_eec_uninf$stage),
                                        margin = 2))
...
barplot_stacked <- ggplot(cp.df, aes(x = timepoint, y = proportion, 
  ...
  barplot_theme + scale_fill_brewer(palette = "Set3") 
```
...
save_ggplot_formats(
  plt = barplot_stacked,
  base_plot_dir = plots_folder,
  plt_name = "barplot_proportions_eec_uninfected",
  width = 3, height = 3
)
```
