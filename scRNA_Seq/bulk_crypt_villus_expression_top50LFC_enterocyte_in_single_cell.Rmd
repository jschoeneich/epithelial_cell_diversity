...
```{r}
table.map <- c(
  preWe.crypts = "pre.weaning.crypts_pre.weaning.villi",
  preWe.villi  = "pre.weaning.crypts_pre.weaning.villi",
  poWe.crypts  = "post.weaning.crypts_post.weaning.villi",
  poWe.villi   = "post.weaning.crypts_post.weaning.villi"
)

new.list.stats <- lapply(names(new.list), function(x) {

  res <- results.list.cv.prepo.filter[[table.map[x]]]

  merge(
    new.list[[x]],
    res[, c("symbol", "log2FoldChange", "padj")],
    by.x = "Symbol",
    by.y = "symbol",
    all.x = TRUE,
    sort = FALSE
...
names(new.list.stats) <- names(new.list)
```

```{r}
write.xlsx(new.list.stats,file =
