---
title: ' RNAseq Analysis Pro/Me/Di - Johannes Schöneich'
output:
  html_document: 
    toc: yes
    fig_width: 12
...
# Read in  countmatrix
## Load in data
countmatrix_raw <- read.csv(file.path(harddrive,
                                      "RNA_Seq/RNAseq_promedi/promedi_fc/counts.csv"),
                            header = TRUE, row.names=1) 
coln <- colnames(countmatrix_raw)
coln2 <- gsub(".markdup.sorted.bam","",coln)
```

```{r}
# As the samples are not numerically ordered, this is the correct order.
# Any correct ordering will occur later
timepoint <- rep(rep(c("d01","d10","d25","d05"),each = 3),3) 
location <- rep(c("dis","med","pro"),each = 12)
coldata <- data.frame("sample_name" = coln2,timepoint,location)
coldata$sample_name <- stri_replace_all_regex(coldata$sample_name,
                                              pattern = c("d1_","d5_","dist","prox"),
                                              replacement = c("d01_","d05_","dis","pro"),
                                              vectorize = FALSE)
```
...
```{r}
#reorder to be sorted by location and time
coldata <- arrange(coldata,location,timepoint)
countmatrix <- countmatrix[,coldata$sample_name]
```
...
```{r}
pcaData <- plotPCA(vsd, intgroup = c("timepoint","location","sample_name"), 
                              returnData = TRUE)

percentVar <- percentvar_calculation(pcaData)

pcaData_pc34 <- plotPCA_34(vsd, intgroup = c("timepoint","location","sample_name"), 
                              returnData = TRUE)
...
fill_colors <- c(
  "pre weaning distal"    = "#B1d1FF",  # Light Blue
  "pre weaning medial"    = "#619CFF",  # Medium Blue (Default, unchanged)
  "pre weaning proximal"  = "#1F5D99", # Even Darker Blue
  "post weaning distal"   = "#FFD6D4",  # Even Lighter Red
  "post weaning medial"   = "#F8766D",  # Medium Red (Default, unchanged)
  "post weaning proximal" = "#C2403C"   # Even Darker Red
)
...
pcaData$condition <- c(rep("pre weaning distal",9),rep("post weaning distal",3),
                       rep("pre weaning medial",9),rep("post weaning medial",3),
                       rep("pre weaning proximal",9),rep("post weaning proximal",3))

location_order <- c("pro", "med", "dis")


# Convert `location` to a factor with the specified order
pcaData$location <- factor(pcaData$location, levels = location_order)
```
...
```{r fig.height = 8, fig.width = 8}
save_ggplot_formats(PCAplot, plot_dir,"PCA_timepoints_circles_v2"
...
```{r}
 save_ggplot_formats(
   ...
   plt_name = glue("pca_biplot_promedi_no_edit"),
...
dds_group <- dds
dds_group$group <- factor(paste0(dds_group$timepoint,".",dds_group$location))
design(dds_group) <- ~ group

dds_early_late <- dds
dds_early_late$stage <- factor(rep(c(rep("prWe",9),rep("poWe",3)),3))

dds_early_late$stage_location <- factor(paste0(dds_early_late$stage,".",dds_early_late$location))
design(dds_early_late) <- ~ stage_location
```
